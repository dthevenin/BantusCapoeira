...
if ( isset($_GET['action']) && isset($_GET['id']) && $_GET['action'] == 'clone') {
	...
	if (!defined('GSNOCSRF') || (GSNOCSRF == FALSE) ) {
		$nonce = $_GET['nonce'];
		if(!check_nonce($nonce, "clone", "moves.php")) {
			die("CSRF detected!");	
		}
	}
	...
	if ($status) {
		...
		if ($status) {
			...
			header('Location: moves.php?upd=clone-success&id='.$newurl);
		} else {
			$error = sprintf(i18n_r('CLONE_ERROR'), $_GET['id']);
			header('Location: moves.php?error='.$error);
		}
	} else {
		$error = sprintf(i18n_r('CLONE_ERROR'), $_GET['id']);
		header('Location: moves.php?error='.$error);
	...
	<?php exec_action('pages-main'); ?>
		<div class="main">
			...
			<div id="filter-search">
				<form><input type="text" autocomplete="off" class="text" id="q" placeholder="<?php echo strip_tags(lowercase(i18n_r('FILTER'))); ?>..." /> &nbsp; <a href="moves.php" class="cancel"><?php i18n('CANCEL'); ?></a></form>
			</div>
			
			<table id="editmoves" class="edittable highlight paginate">
				<tr><th><?php i18n('PAGE_TITLE'); ?></th><th style="text-align:right;" ><?php i18n('DATE'); ?></th><th></th><th></th></tr>
				<?php echo $table; ?>
			</table>
			<p><em><b><span id="pg_counter"><?php echo $count; ?></span></b> <?php i18n('TOTAL_MOVES'); ?></em></p>
