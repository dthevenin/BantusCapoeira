 ...
 * Page Edit
 *
 * Edit or create new moves for the website.	
 *
 * @package GetSimple
...
if ($id){
	...
	if (!file_exists($path . $file)){ 
		redirect('moves.php?error='.urlencode(i18n_r('PAGE_NOTEXIST')));
	}
...
<div class="bodycontent clearfix">
	...
	<div id="maincontent">
		...
		<h3 class="floated"><?php if(isset($data_edit)) { i18n('MOVE_EDIT_MODE'); } else { i18n('CREATE_NEW_MOVE'); } ?></h3>	
		...
		<form class="largeform" id="editform" action="changedata.php" method="post" accept-charset="utf-8" >
			<input id="nonce" name="nonce" type="hidden" value="<?php echo get_nonce("edit", "editmove.php"); ?>" />			
			...
			<div id="submit_line" >
				...
				<div id="dropdown">
					...
					<ul class="dropdownmenu">
						...
						<?php if($url != '') { ?>
							<li><a href="moves.php?id=<?php echo $url; ?>&amp;action=clone&amp;nonce=<?php echo get_nonce("clone","moves.php"); ?>" ><?php i18n('CLONE'); ?></a></li>
						<?php } ?>
						<li id="cancel-updates" class="alertme"><a href="moves.php?cancel" ><?php i18n('CANCEL'); ?></a></li>
			...
			<?php if($url != '') { ?>
				<p class="backuplink" ><?php 
					...
					if ( file_exists(GSBACKUPSPATH.'pages/'.$url.'.bak.xml') ) {	
						echo '&bull;&nbsp;&nbsp; <a href="backup-editmove.php?p=view&amp;id='.$url.'" target="_blank" >'.i18n_r('BACKUP_AVAILABLE').'</a>';
					} 
				?></p>
			<?php } ?>
	...
	<div id="sidebar" >
		<?php include('template/sidebar-moves.php'); ?>	
	</div>
